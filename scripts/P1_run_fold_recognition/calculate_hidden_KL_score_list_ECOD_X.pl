...
for $name (keys %protein2label)
{
  ...
  open(IN,"$filepath") || die "Failed to open file $filepath\n";
	...
	if(@tmp2!=1700)
	{
		die "The number of train prediciton is not correct ".@test_feas." != ".@tmp2." in $filepath\n";
	}
...
for $name (keys %run_proteinlist)
{
  ...
  open(IN,"$filepath") || die "Failed to open file $filepath\n";
	...
	if(@tmp2!=1700)
	{
		die "The number of test prediciton is not correct 1700 != ".@tmp2." in $filepath\n";
	}	
	...
	if(@test_feas!=1700)
	{
		die "The number of prediciton is not correct ".@test_feas." != ".@tmp2." in $filepath\n";
	}
	...
	open(OUT1,">$outdir/$name.KL_output")|| die("Failed to open file $outdir/$name.KL_output \n");
