...
sub create_feature
{
	...
	foreach $pir_file (@files)
	{
		...
		print("$prosys_dir/pir2ts_energy.pl $modeller_dir $atom_dir $work_dir $pir_file $num_model_simulate 2>/dev/null\n");
		system("$prosys_dir/pir2ts_energy.pl $modeller_dir $atom_dir $work_dir $pir_file $num_model_simulate 2>/dev/null");
