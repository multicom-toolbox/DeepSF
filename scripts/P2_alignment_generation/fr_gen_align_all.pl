...
while (@ids)
{
	...
	elsif ($alignment_method eq "lobster")
	{
		...
		print("$script_dir/coach.pl $lobster_dir $query_file $query_fas $tname.fasta $temp_fas $temp_lob $tname.res > /dev/null\n"); 
	...
	elsif ($alignment_method eq "muscle")
	{
		...
		#system("$script_dir/coach.pl $lobster_dir $query_file $query_fas $tname.fasta $temp_fas $temp_lob $tname.res > /dev/null"); 
		print("$script_dir/muscle.pl $lobster_dir $query_file $query_fas $tname.fasta $temp_fas $temp_lob $tname.res > /dev/null\n"); 
		system("$script_dir/muscle.pl $lobster_dir $query_file $query_fas $tname.fasta $temp_fas $temp_lob $tname.res > /dev/null"); 
		#open(RES, "$tname.res") || die "can't read clustalw profile alignments.\n";
		#@res = <RES>;
		#close RES;
...
#		system("$compass_dir/new_compass/prep_psiblastali -i $query_msa -o $query_aln");
		...
		-f $temp_msa || die "can't find template msa file: $temp_msa\n"; 
		print("$script_dir/new_compass.pl $compass_dir $query_file $query_msa $tname.fasta $temp_msa $tname.res >/dev/null\n"); 
	...
	elsif ($alignment_method eq "hhsearch")
	{
		...
		if(-e "$template_dir/$tname.shhm")
		{
			`cp $template_dir/$tname.shhm ./$tname.shhm`;
		}elsif(-e "$template_dir/$tname.hhm")
		{
			`cp $template_dir/$tname.hhm ./$tname.shhm`;
		}else{
			print "Failed to find $template_dir/$tname.shhm or $template_dir/$tname.hhm, pass\n\n";
			next;
		}
		
		print("$script_dir/hhsearch_align.pl $prosys_dir $new_hhsearch_dir $qname.shhm $tname.shhm $query_file $tname.pir\n");	
